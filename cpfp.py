...
parser.add_argument('--txid', action='store', type=str,
                    help='Transaction id to double-spend')
...
if args.txid is not None:
    # address should be the signing role
    rawTx = rpc.getrawtransaction(lx(args.txid), False)
    decTx = CMutableTransaction.from_tx(rawTx)
    value_in = 0
    value_out = 0
    for vin in decTx.vin:
        rawVinTx = rpc.getrawtransaction(vin.prevout.hash, False)
        decRawVinTx = CMutableTransaction.from_tx(rawVinTx)
        value_in += decRawVinTx.vout[vin.prevout.n].nValue

    # calculate the previous fee
    value_out = sum([vout.nValue for vout in decTx.vout])
    delta_fee = value_in - value_out
    logging.debug('Previous fee: %s' % str_money_value(value_in - value_out))

    tx_n = 0
    for idx, vout in enumerate(decTx.vout):
        nValue = vout.nValue
        outAddress = str(CBitcoinAddress.from_scriptPubKey(vout.scriptPubKey))
    ...
    tx = CMutableTransaction()
    new_outpoint = COutPoint(lx(args.txid), tx_n)
    new_txin = CMutableTxIn(new_outpoint, nSequence=tx_n)
    tx.vin.append(new_txin)

    new_txout = CMutableTxOut(int(args.amount * COIN), payment_address.to_scriptPubKey())
    tx.vout.append(new_txout)

    delta_fee = int(feeperbyte2 * len(tx.serialize())) - delta_fee
    value_out -= delta_fee
    new_txout.nValue = value_out
